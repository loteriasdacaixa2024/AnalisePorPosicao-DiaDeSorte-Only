/**
 * Painel Estratégico — métricas de conferências (extraído da Central de Conferências).
 * Espera IDs de KPI/canvas definidos no partial painel_estrategico_metricas_pane.html.
 */
(function () {
    'use strict';
    ...
    function renderizarGraficoEficiencia(dados) {
        const canvas = document.getElementById('chartEvolucaoEficiencia');
    ...
    function renderizarGraficoRedundancia(dados) {
        ...
        const canvas = document.getElementById('chartRedundancia');
        if (!canvas || typeof Chart === 'undefined') return;
        const ctx = canvas.getContext('2d');
        if (chartRedundancia) chartRedundancia.destroy();

        let cor = '#2ecc71';
        if (valorRedundancia > 1.5) cor = '#f1c40f';
        if (valorRedundancia > 2.0) cor = '#e74c3c';
    ...
    document.addEventListener('DOMContentLoaded', function () {
        const tab = document.getElementById('tab-painel-estrategico-estrutura');
        if (tab) {
