// Configuração CORRIGIDA - SOMENTE APIs que funcionam (27 análises)
// Dashboard de Análises - Dia de Sorte
// Removidas todas as análises que retornam 404 ou 405
...
const ANALISES_CONFIG = {
    ...
    'matriz-probabilidade': {
        ...
        processar: (data) => {
            // FIX: Verificar se matriz é array antes de usar slice
            const matriz = data.matriz || data.probabilidades || [];

            if (!Array.isArray(matriz)) {
                return {
                    top3: [],
                    insight: 'Matriz de probabilidades disponível. Use para apostas direcionadas.'
                };
            }
...
// Total: 27 análises VALIDADAS que funcionam
// Removidas (404 errors): atrasados, combinacoes, capicua, dezenas, quadrantes, gaps, consecutivos, espelhados, repeticoes, numeros-atrasados, meses-atrasados, gaps-completo, gaps-expandido, valores-probabilidades
// Removidas (405 POST only): calculadora-probabilidade, simulador-apostas
// Removidas (config não encontrada): expectativa-meses
