// Configuração de TODAS as 43 Análises do Sistema
...
const ANALISES_CONFIG = {
    ...
    'combinacoes': {
        ...
        processar: (data) => {
            const combinacoes = data.combinacoes || data.top_combinacoes || [];
            const top3 = combinacoes.slice(0, 3).map(c => ({
                descricao: c.descricao || `${c.numeros ? c.numeros.join(', ') : c.combinacao}`,
                percentual: c.percentual || c.frequencia
            }));
            const insight = combinacoes[0]
                ? `A combinação mais frequente apareceu ${combinacoes[0].frequencia || combinacoes[0].percentual}% das vezes. Use combinações históricas para aumentar suas chances.`
                : 'Sem dados disponíveis.';
            return { top3, insight };
        }
    },
    ...
    'numeros-juntos': {
        ...
        processar: (data) => {
            const juntos = data.numeros_juntos || data.pares || [];
            const top3 = juntos.slice(0, 3).map(j => ({
                descricao: `${j.numero1}-${j.numero2}` || j.descricao,
                percentual: j.percentual || j.frequencia
            }));
            const insight = juntos[0]
                ? `O par ${juntos[0].numero1}-${juntos[0].numero2} aparece junto com frequência. Considere duplas históricas.`
                : 'Sem dados disponíveis.';
            return { top3, insight };
        }
    },
    ...
    'gaps-completo': {
        titulo: 'GAPS Completo',
        icone: 'fa-ruler-combined',
        descricao: 'Análise completa de gaps e cruzamentos',
        rota: '/api/analise/gaps-completo',
        processar: (data) => {
            const padroes = data.padroes || data.gaps || [];
            const top3 = padroes.slice(0, 3).map(p => ({
                descricao: p.descricao || p.padrao,
                percentual: p.percentual
            }));
            const insight = padroes[0]
                ? `Análise completa de gaps mostra padrão ${padroes[0].descricao || padroes[0].padrao}.`
                : 'Sem dados disponíveis.';
            return { top3, insight };
        }
    },
    ...
    'gaps-expandido': {
        titulo: 'GAPS Expandido',
        icone: 'fa-expand-arrows-alt',
        descricao: 'Análise expandida de distâncias',
        rota: '/api/analise/gaps-expandido',
        processar: (data) => {
            const padroes = data.padroes || [];
            const top3 = padroes.slice(0, 3).map(p => ({
                descricao: p.descricao || p.padrao,
            ...
            const insight = padroes[0]
                ? `Análise expandida: ${padroes[0].descricao || padroes[0].padrao} (${padroes[0].percentual}%).`
                : 'Sem dados disponíveis.';
            return { top3, insight };
        }
    },
    ...
    'expectativa-meses': {
        titulo: 'Expectativa de Meses',
        icone: 'fa-calendar-check',
        descricao: 'Expectativa estatística dos meses',
        rota: '/api/analise/expectativa-meses',
        processar: (data) => {
            const expectativas = data.expectativas || data.meses || [];
            const top3 = expectativas.slice(0, 3).map(e => ({
                descricao: `${e.nome || e.mes}`,
                percentual: e.expectativa || e.percentual
            }));
            const insight = expectativas[0]
                ? `Maior expectativa: ${expectativas[0].nome || expectativas[0].mes}. Use para apostas preditivas.`
                : 'Sem dados disponíveis.';
            return { top3, insight };
        }
    },
    ...
    'calculadora-probabilidade': {
        titulo: 'Calculadora de Probabilidade',
        icone: 'fa-percentage',
        descricao: 'Cálculo de probabilidades estatísticas',
        rota: '/api/analise/calculadora-probabilidade',
        processar: (data) => {
            const calculos = data.calculos || data.probabilidades || [];
            const top3 = calculos.slice(0, 3).map(c => ({
                descricao: c.descricao || `Cálculo ${c.tipo}`,
                percentual: c.probabilidade || c.percentual
            }));
            const insight = data.probabilidade_geral
                ? `Probabilidade geral: ${data.probabilidade_geral}%. Use para dimensionar apostas.`
                : 'Ferramenta de cálculo disponível.';
            return { top3, insight };
        }
    },
    ...
    'simulador-apostas': {
        titulo: 'Simulador de Apostas',
        icone: 'fa-dice',
        descricao: 'Simulação de apostas e cenários',
        rota: '/api/analise/simulador-apostas',
        processar: (data) => {
            const simulacoes = data.simulacoes || [];
            const top3 = simulacoes.slice(0, 3).map(s => ({
                descricao: s.descricao || `Cenário ${s.id}`,
                percentual: s.taxa_acerto || s.score
            }));
            const insight = data.melhor_cenario
                ? `Melhor cenário: ${data.melhor_cenario.descricao}. Simule antes de apostar!`
