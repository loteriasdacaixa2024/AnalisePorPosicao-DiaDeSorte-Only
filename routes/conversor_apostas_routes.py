...
def download_json():
    ...
    try:
        ...
        # Formatar JSON customizado - cada aposta em uma linha COM ESPAÇOS
        json_lines = ['{\n']
        json_lines.append(f'  "concurso": {dados["concurso"]},\n')
        json_lines.append('  "apostas": [\n')

        for index, aposta in enumerate(dados.get('apostas', [])):
            is_last = index == len(dados['apostas']) - 1

            # Formatar manualmente para garantir espaços após : e ,
            numeros_formatados = ', '.join(map(str, aposta['numeros']))
            aposta_linha = f'{{"numero": {aposta["numero"]}, "numeros": [{numeros_formatados}], "mes": "{aposta["mes"]}"}}'

            json_lines.append(f'    {aposta_linha}{"" if is_last else ","}\n')
