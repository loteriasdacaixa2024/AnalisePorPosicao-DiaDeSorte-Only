...
class DesdobramentoService:
    ...
    def gerar_jogos_modelo_e(sorteio_base, complementares, nivel=2, mes='Jan', dezenas_por_jogo=7):
        ...
        # Buscar preço correto baseado na quantidade de dezenas
        from services.configuracao_service import ConfiguracaoService
        custo_unitario = ConfiguracaoService.obter_preco_por_dezenas(dezenas_por_jogo)
        custo_total = total_jogos * custo_unitario
    ...
    # ========================================================================
    # MODELO F: BAIXA CONECTIVIDADE + MAX 2 REPETIDAS
    # ========================================================================

    @staticmethod
    def gerar_sugestao_modelo_f():
        """
        Gera sugestão automática para Modelo F.
        Retorna:
        - ultimo_sorteio: lista de 7 números
        - base_sugerida: 5 números favoritos (baseados em frequência/atraso)
        """
        # Buscar último sorteio
        ultimo_sorteio = Sorteio.query.order_by(Sorteio.concurso.desc()).first()
        ...
        nums_ultimo = sorted(ultimo_sorteio.get_posicoes_lista())


        # Análise para sugerir base (5 números) que NÃO estejam no último sorteio
        sorteios = Sorteio.query.order_by(Sorteio.concurso.desc()).all()
        analise = DesdobramentoService._analisar_frequencias(sorteios)
        
        # 'quentes' e 'medios' retornados por _analisar_frequencias são LISTAS de números (ints)
        # Verificamos se há dados
        quentes = analise.get('quentes', [])
        medios = analise.get('medios', [])
        
        # Pega os mais quentes que NÃO estão no último sorteio
        candidatos = [n for n in quentes if n not in nums_ultimo]
        
        # Se não tiver quentes suficientes, completa com médios
        if len(candidatos) < 5:
            candidatos += [n for n in medios if n not in nums_ultimo and n not in candidatos]
            
        # Se ainda não der 5, pega dos frios
        if len(candidatos) < 5:
             frios = analise.get('frios', [])
             candidatos += [n for n in frios if n not in nums_ultimo and n not in candidatos]

        # Se mesmo assim não der (banco vazio?), preenche com 1-5 que não esteja no ultimo
        if len(candidatos) < 5:
            for i in range(1, 32):
                if i not in nums_ultimo and i not in candidatos:
                    candidatos.append(i)
        
        base_sugerida = sorted(candidatos[:5])

        return {
            'ultimo_sorteio': nums_ultimo,
            'base_sugerida': base_sugerida,
            'concurso': ultimo_sorteio.concurso
        }

    @staticmethod
    def gerar_jogos_modelo_f(ultimo_sorteio, base_preferida, mes='Jan'):
        """
        Gera jogos do Modelo F:
        - Garante Max 2 do último sorteio (C(7,2) = 21 jogos)
        - Baixa Conectividade usando Rotação de Base
        """
        from itertools import combinations
        import random

        if len(ultimo_sorteio) != 7:
            return {'erro': 'O último sorteio deve ter 7 números.'}
        
        if len(base_preferida) != 5:
            return {'erro': 'A base preferida deve ter 5 números.'}

        # 1. Gerar os 21 pares obrigatórios do último sorteio
        pares_obrigatorios = list(combinations(sorted(ultimo_sorteio), 2))

        # 2. Preparar pool de rotação (Base 5 + 5 Extras)
        # Os extras devem ser números que não estão no último sorteio nem na base
        todos = set(range(1, 32))
        usados = set(ultimo_sorteio) | set(base_preferida)
        disponiveis = sorted(list(todos - usados))
        
        # Selecionar 5 extras (pode ser os mais frequentes ou aleatórios)
        # Vamos usar aleatórios com seed para garantir mistura, mas determinístico se quiser
        # Aqui vamos pegar os 5 primeiros disponíveis para simplificar ou aleatório
        random.seed(42) # Seed fixa para reprodutibilidade básica
        extras = random.sample(disponiveis, 5) if len(disponiveis) >= 5 else disponiveis[:5]
        
        pool_rotacao = base_preferida + extras
        
        jogos = []
        
        for idx, par in enumerate(pares_obrigatorios, 1):
            # Começa com o par obrigatório (2 números)
            aposta = list(par)
            
            # Filtra o pool de rotação para remover qualquer número que já esteja no par (aposta)
            # Removemos também duplicatas criando um conjunto
            pool_limpo = list(set(x for x in pool_rotacao if x not in aposta))
            
            # Se a limpeza reduzir o pool para menos de 5 opções, compõe com dezenas disponíveis
            if len(pool_limpo) < 5:
                faltantes = 5 - len(pool_limpo)
                disp_agora = [x for x in disponiveis if x not in pool_limpo and x not in aposta]
                # Seed temporária baseada no par para aleatoriedade deterministica sem repetições
                random.seed(42 + idx)
                if len(disp_agora) >= faltantes:
                    extras_agora = random.sample(disp_agora, faltantes)
                else:
                    extras_agora = disp_agora
                pool_limpo.extend(extras_agora)
            
            # Mistura o pool válido e pega 5 para formar as 7 dezenas
            random.seed(42 + idx) # Reseed dinâmico baseado no index para evitar repetições padronizadas entre pares
            complemento = random.sample(pool_limpo, 5)
            
            numeros_completos = sorted(aposta + complemento)
            
            jogos.append({
                'numero': idx,
                'fixas': [], # Não tem fixas reais neste modelo
                'variaveis': numeros_completos, # Tudo é variável na prática
                'numeros_completos': numeros_completos,
                'mes': mes
            })
        ...
        return {
            ...
            'custo_total': custo_total,
            'modelo': 'F',
            'base_preferida': base_preferida,
            'extras_usados': extras,
            'mes': mes
        }

    @staticmethod
    def gerar_jogos_modelo_g(dezenas_selecionadas, mes='Jan'):
        """
        Gera jogos do Modelo G (Redução de 16 dezenas para 2 jogos de 8 dezenas cada)
        
        Lógica:
        - Seleciona exatamente 16 dezenas
        - Divide de maneira alternada (pares e ímpares por índice) após ordenação
        - Gera exatamente 2 jogos
        """
        if len(dezenas_selecionadas) != 16:
            return {'erro': 'Você deve escolher exatamente 16 dezenas para o Modelo G.'}

        dezenas_ordenadas = sorted(list(set(dezenas_selecionadas)))
        if len(dezenas_ordenadas) != 16:
            return {'erro': 'Não são permitidas dezenas duplicadas.'}

        # Separar alternadamente
        jogo1 = [dezenas_ordenadas[i] for i in range(0, 16, 2)]
        jogo2 = [dezenas_ordenadas[i] for i in range(1, 16, 2)]

        jogos = [
            {
                'numero': 1,
                'fixas': [],
                'variaveis': sorted(jogo1),
                'numeros_completos': sorted(jogo1),
                'mes': mes
            },
            {
                'numero': 2,
                'fixas': [],
                'variaveis': sorted(jogo2),
                'numeros_completos': sorted(jogo2),
                'mes': mes
            }
        ]
        ...
        # Custo unitário de aposta de 8 dezenas no Dia de Sorte
        try:
            from services.configuracao_service import ConfiguracaoService
            custo_unitario = ConfiguracaoService.obter_preco_por_dezenas(8)
        except Exception:
            custo_unitario = 20.00  # Custo oficial padrão para 8 dezenas (8 x R$ 2.50)
        ...
        return {
            ...
            'custo_total': custo_total,
            'modelo': 'G',
            'dezenas_selecionadas': dezenas_ordenadas,
            'mes': mes
        }
