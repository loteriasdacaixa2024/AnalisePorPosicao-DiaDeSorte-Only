...
class ConfiguracaoService:
    ...
    def _atualizar_sorteio_db(data_api):
        ...
        try:
            ...
            # Dezenas (ordem de sorteio)
            lista_ordem_sorteio = data_api.get('dezenasSorteadasOrdemSorteio', [])
            if len(lista_ordem_sorteio) >= 7:
                sorteio.sorteio_1 = int(lista_ordem_sorteio[0])
                sorteio.sorteio_2 = int(lista_ordem_sorteio[1])
                sorteio.sorteio_3 = int(lista_ordem_sorteio[2])
                sorteio.sorteio_4 = int(lista_ordem_sorteio[3])
                sorteio.sorteio_5 = int(lista_ordem_sorteio[4])
                sorteio.sorteio_6 = int(lista_ordem_sorteio[5])
                sorteio.sorteio_7 = int(lista_ordem_sorteio[6])
    ...
    def salvar_cores_meses(cores_dict):
        ...
        try:
            ...
            if erros and sucessos == 0:
                return {
                    'sucesso': False,
                    'mensagem': 'Erros ao salvar cores: ' + ', '.join(erros)
                }
            elif erros:
                return {
                    'sucesso': True,
                    'mensagem': f'{sucessos} cores salvas. Alguns erros: ' + ', '.join(erros)
                }
            else:
                return {
                    'sucesso': True,
                    'mensagem': f'Todos os {sucessos} cores salvas com sucesso!'
                }
    ...
    # ========================================================================
    # PREÇOS POR QUANTIDADE DE DEZENAS (7-15)
    # ========================================================================
    ...
    @staticmethod
    def obter_preco_por_dezenas(quantidade_dezenas):
        """
        Obtém o preço de uma aposta com determinada quantidade de dezenas

        Args:
            quantidade_dezenas: Número de dezenas (7 a 15)

        Returns:
            float: Preço da aposta
        """
        if quantidade_dezenas < 7 or quantidade_dezenas > 15:
            return 2.50  # Valor padrão

        chave = f'preco_dezenas_{quantidade_dezenas}'
        preco_padrao = ConfiguracaoService.PRECOS_OFICIAIS.get(quantidade_dezenas, 2.50)

        return ConfiguracaoService.obter_configuracao(chave, preco_padrao)

    @staticmethod
    def obter_todos_precos_dezenas():
        """
        Obtém todos os preços por quantidade de dezenas (7 a 15)

        Returns:
            dict: {7: preco7, 8: preco8, ..., 15: preco15}
        """
        precos = {}

        for dezenas in range(7, 16):
            precos[dezenas] = ConfiguracaoService.obter_preco_por_dezenas(dezenas)

        return precos

    @staticmethod
    def salvar_precos_dezenas(precos_dict):
        """
        Salva os preços por quantidade de dezenas

        Args:
            precos_dict: Dicionário {dezenas: preco, ...}
        ...
        try:
            ...
            for dezenas_str, preco in precos_dict.items():
                try:
                    dezenas = int(dezenas_str)

                    if dezenas < 7 or dezenas > 15:
                        erros.append(f"Dezenas {dezenas} inválido (deve ser 7-15)")
                        continue

                    preco_float = float(preco)

                    if preco_float < 0:
                        erros.append(f"Preço para {dezenas} dezenas não pode ser negativo")
                        continue

                    chave = f'preco_dezenas_{dezenas}'
                    resultado = ConfiguracaoService.salvar_configuracao(
                        chave=chave,
                        valor=preco_float,
                        tipo='float',
                        descricao=f'Preço da aposta com {dezenas} dezenas'
                    )

                    if resultado['sucesso']:
                        sucessos += 1
                    else:
                        erros.append(f"Dezenas {dezenas}: {resultado['mensagem']}")

                except Exception as e:
                    erros.append(f"Dezenas {dezenas_str}: {str(e)}")

            if erros and sucessos == 0:
                return {
                    'sucesso': False,
                    'mensagem': 'Erros ao salvar preços: ' + ', '.join(erros)
                }
            elif erros:
                return {
                    'sucesso': True,
                    'mensagem': f'{sucessos} preços salvos. Alguns erros: ' + ', '.join(erros)
                }
            else:
                return {
                    'sucesso': True,
                    'mensagem': f'Todos os {sucessos} preços salvos com sucesso!'
                }
    ...
    @staticmethod
    def restaurar_precos_oficiais():
        """
        Restaura todos os preços para os valores oficiais da Caixa

        Returns:
            dict com resultado da operação
        """
        return ConfiguracaoService.salvar_precos_dezenas(ConfiguracaoService.PRECOS_OFICIAIS)

    # ========================================================================
    # ANALYSIS COLORS CONFIGURATION METHODS
    # ========================================================================

    @staticmethod
    def obter_cores_analise():
        """
        Obtém as 7 cores de análise configuradas
        """
        return {
            'repetidos': ConfiguracaoService.obter_configuracao('cor_analise_repetidos', '#6f42c1'),
            'sequencia_1': ConfiguracaoService.obter_configuracao('cor_analise_sequencia_1', '#000000'),
            'sequencia_2': ConfiguracaoService.obter_configuracao('cor_analise_sequencia_2', '#555555'),
            'sequencia_3': ConfiguracaoService.obter_configuracao('cor_analise_sequencia_3', '#999999'),
            'pares': ConfiguracaoService.obter_configuracao('cor_analise_pares', '#28a745'),
            'impares': ConfiguracaoService.obter_configuracao('cor_analise_impares', '#17a2b8'),
            'finais_iguais': ConfiguracaoService.obter_configuracao('cor_analise_finais_iguais', '#ffc107')
        }

    @staticmethod
    def salvar_cores_analise(cores_dict):
        """
        Salva as cores de análise
        """
        try:
            erros = []
            for chave, valor in cores_dict.items():
                chave_db = f'cor_analise_{chave}'
                resultado = ConfiguracaoService.salvar_configuracao(chave_db, str(valor))
                if not resultado['sucesso']:
                    erros.append(resultado['mensagem'])
            
            return {
                'sucesso': len(erros) == 0,
                'mensagem': 'Cores de análise salvas com sucesso',
                'erros': erros
            }
        except Exception as e:
    ...
    @staticmethod
    def restaurar_cores_analise_padrao():
        """
        Restaura as cores de análise para os valores padrão
        """
        cores_padrao = {
            'repetidos': '#6f42c1',
            'sequencia_1': '#000000',
            ...
            'pares': '#28a745',
            'impares': '#17a2b8',
            'finais_iguais': '#ffc107'
        }
        return ConfiguracaoService.salvar_cores_analise(cores_padrao)
