...
:: Painel 1 = servidor | Painel 2 = extrator (mesmo Edge com login auto)
"%LOCALAPPDATA%\Microsoft\WindowsApps\wt.exe" ^
  nt -d "%RAIZ%" cmd /k "title SERVIDOR & echo ======================================== & echo  SERVIDOR PRINCIPAL ^(%SERVIDOR_PY%^) & echo ======================================== & echo Pasta: %RAIZ% & echo. & echo Pressione ENTER para ligar... & pause >nul & \"%PYTHON_EXE%\" \"%RAIZ%\%SERVIDOR_PY%\"" ^
  ; split-pane -H -d "%PASTA_BOLOES%" cmd /k "title EXTRATOR & set LOGIN_CAIXA_AUTO=1 & echo ======================================== & echo  EXTRATOR DE BOLOES - Caixa ^(API^) & echo  VERSAO: CHECKPOINT-RESUME-v3.1 & echo ======================================== & echo LOGIN AUTOMATICO: ligado ^(mesmo Edge^) & echo   1^) Digite o codigo do e-mail no Edge & echo   2^) Clique MANUAL em Enviar & echo   3^) Depois ENTER/SIM no terminal & echo Credenciais: config.local.json ^(gitignore^) & echo. & echo Pressione ENTER para rodar... & pause >nul & \"%PYTHON_EXE%\" -u \"%PASTA_BOLOES%\script\baixar_boloes-API.py\""
