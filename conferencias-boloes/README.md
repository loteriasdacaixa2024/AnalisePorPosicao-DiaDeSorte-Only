...
## 🔁 Checkpoint / Retomada (extração API)

O extrator `script/baixar_boloes-API.py` grava progresso em:

`json-boloes/_checkpoint_extracao.json`

Após cada página concluída. Se interromper (ou pressionar **P** para pausar):

1. Rode o extrator de novo (mesmo fluxo de login);
2. O terminal pergunta: **[C] Continuar** ou **[N] Nova**;
3. Em Continuar, navega para a **próxima página** (ex.: parou em 16 → retoma em 17).

Arquivo canônico dos bolões: `boloes_{concurso}_{modalidade}.json`  
(o espelho `*_CONSOLIDADO.json` deixou de ser gravado a cada página — era duplicata).
